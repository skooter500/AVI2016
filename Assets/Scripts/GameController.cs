...
namespace Topology {
	...
	public class GameController : MonoBehaviour {
		...
		public static Transform nodeCenter;
		private Vector3 edgeCenter = new Vector3(0,0,0);
		//Method for loading the GraphML layout file
		private IEnumerator LoadLayout(){

			string sourceFile = Application.dataPath + "/Data/layoutSmall.xml";
			...
			for(int i=0; i<root.ChildNodes.Count; i++){
				...
				for(int j=0; j<xmlGraph.ChildNodes.Count; j++){
					...
					if(xmlNode.Name == "node"){
						...
						nodeObject.transform.SetParent(newParent);
						...
						nodeCount++;
						edgeCenter+=nodeObject.transform.position;
					...
					if(xmlNode.Name == "edge"){
						...
						linkObject.transform.SetParent(newParent);
			...
			//map node edges
			MapLinkNodes();
			edgeCenter/=nodeCount;
			nodeCenter.position = edgeCenter;
		...
		void Start () {
			nodeCenter = GameObject.Find ("graphcenterCollider").transform;
			nodes = new Hashtable();
			links = new Hashtable();
